...
bool ObjManagement::insertAttributeToNode(string ID, string key, string value)
{
	...
	bool flag = true;
		...
		Json::Value attribute;//要更新的内容
		attribute["ATTRIBUTE"] = key;
		attribute["VALUE"] = value;
		reader.parse(jsonstr, json_object, false);
		json_object1 = json_object["AVP_LIST"];
		json_object2 = json_object["ID"];
		 json_object1.append(attribute);//向"AVP_LIST"中添加要更新的内容
		 json_object.removeMember("AVP_LIST");//删除原来的“AVP_LIST”
		 json_object["AVP_LIST"].append(json_object1);//添加新的“AVP_LIST”
